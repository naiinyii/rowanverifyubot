"""PNG student证generatemodule - Penn State LionPATH"""
import random
...
def generate_html(first_name, last_name, school_id='2565'):
    ...
    Args:
        first_name: first name字
        last_name: last name氏
        school_id: school ID
...
<head>
    ...
    <style>
        ...
        body {{
            ...
            background-color: #e0e0e0; /* 浏览器background */
            margin: 0;
            padding: 20px;
...
def generate_image(first_name, last_name, school_id='2565'):
    ...
    try:
        ...
        # 使用 Playwright Screenshot(替代 Selenium)
        with sync_playwright() as p:
            browser = p.chromium.launch(headless=True)
            page = browser.new_page(viewport={'width': 1200, 'height': 900})
            page.set_content(html_content, wait_until='load')
            page.wait_for_timeout(500)  # waitstyleloading
...
if __name__ == '__main__':
    ...
    # 修复 Windows 控制台encodingissue
    if sys.platform == 'win32':
        sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')

    print("test PSU graph片generate...")

    first_name = "John"
    last_name = "Smith"

    print(f"last namefirst name: {first_name} {last_name}")
    ...
    try:
        ...
        print(f"✓ graph片generatesuccess! size: {len(img_data)} bytes")
        print("✓ 已save为 test_psu_card.png")
