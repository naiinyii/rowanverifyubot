"""SheerID Teacher Verification Main Program(Bolt.new)"""
import re
import random
import logging
import httpx
...
class SheerIDVerifier:
    ...
    def normalize_url(url: str) -> str:
        """Normalize URL(keep as is,compatible with existing interface)"""
        return url
    ...
    def create_verification(self) -> str:
        """Request new verificationId via installPageUrl"""
        body = {
        ...
        if status != 200 or not isinstance(data, dict) or not data.get("verificationId"):
            raise Exception(f"Failed to create verification (status code {status}): {data}")

        self.verification_id = data["verificationId"]
        logger.info(f"✅ Get verificationId: {self.verification_id}")
    ...
    def verify(
        ...
        """executeteachervalidationworkflow"""
        try:
            ...
            # generateteacher PNG
            logger.info("step 1/5: generateteacher PNG 文档...")
            ...
            # committeacherinformation
            logger.info("step 2/5: committeacherinformation...")
            step2_body = {
            ...
            logger.info(f"✅ step 2 complete: {getattr(step2_data, 'get', lambda k, d=None: d)('currentStep')}")
            current_step = (
            ...
            if current_step in ["sso", "collectTeacherPersonalInfo"]:
                logger.info("step 3/5: skip SSO validation...")
                step3_data, _ = self._sheerid_request(
                    "DELETE",
                    f"{config.SHEERID_BASE_URL}/rest/v2/verification/{self.verification_id}/step/sso",
                )
                logger.info(f"✅ step 3 complete: {getattr(step3_data, 'get', lambda k, d=None: d)('currentStep')}")
                current_step = (
                    step3_data.get("currentStep", current_step) if isinstance(step3_data, dict) else current_step
                )

            # Requestupload并upload文档
            logger.info("step 4/5: Requestupload URL ...")
            step4_body = {
                "files": [
                    {
            ...
            if step4_status != 200 or not isinstance(step4_data, dict) or not step4_data.get("documents"):
                raise Exception(f"未能fetchupload URL: {step4_data}")

            documents = step4_data["documents"]
            if len(documents) != len(assets):
                raise Exception("back的uploadtaskcount与filecount不匹配")

            for doc, asset in zip(documents, assets):
                upload_url = doc.get("uploadUrl")
                if not upload_url:
                    raise Exception("缺少upload URL")
                if not self._upload_to_s3(upload_url, asset["data"]):
                    raise Exception(f"S3 uploadfailure: {asset['file_name']}")
            ...
            logger.info(f"✅ 文档commitcomplete: {getattr(step6_data, 'get', lambda k, d=None: d)('currentStep')}")

            # fetch最终状态(contain rewardCode)
...
def main():
    ...
    print("=" * 60)
    print("SheerID teacheridentityvalidationtools (Python版)")
    print("=" * 60)
    print()
    ...
    if len(sys.argv) > 1:
        ...
        url = input("请输入 SheerID validation入口链接 (含 externalUserId): ").strip()
