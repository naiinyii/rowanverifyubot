"""PNG Teacher Certificate Generation Module - Bolt.new / PSU"""
import random
from datetime import datetime


def generate_psu_id():
    """Generate random PSU ID (9digits)"""
...
def generate_psu_email(first_name, last_name):
    """
    Generate PSU email
    Format: firstName.lastName + 3-4digits @psu.edu
    """
...
def _get_browser_context():
    ...
    if _browser_context is None:
        try:
            ...
            raise Exception("Need to install playwright: pip install playwright && playwright install chromium")
    return _browser_context


def _html_to_png(html_content: str, width: int = 1200, height: int = None) -> bytes:
    """Convert HTML to PNG screenshot(Optimized version:Reuse browser instance)"""
    try:
        context = _get_browser_context()
        page = context.new_page()

        try:
            # Directly set HTML content,Use domcontentloaded instead of networkidle(faster)
            page.set_content(html_content, wait_until='domcontentloaded')

            # Wait for image loading(if there are external images)
...
def generate_teacher_card_html(first_name: str, last_name: str, psu_id: str) -> str:
    """Generate teacher certificate HTML."""
...
def generate_employment_letter_html(
    ...
    """Generate teacher employment certificate HTML."""
    name = f"{first_name} {last_name}"
    now = datetime.now()
...
def _html_to_png_batch(html_list: list[tuple[str, int, int]]) -> list[bytes]:
    """
    Batch concurrent generation of multiple PNGs(performanceOptimized version)

    Args:
        html_list: [(html_content, width, height), ...]

    Returns:
        list[bytes]: PNG data list
    """
    import asyncio
    from playwright.async_api import async_playwright

    async def render_single(html_content: str, width: int, height: int):
        """Asynchronously render single image"""
...
def generate_images(first_name: str, last_name: str, school_id: str = '2565'):
    """
    Generate two PNGs:teacher card + employment certificate(concurrentOptimized version)

    Args:
        first_name: first name
        last_name: last name
        school_id: school ID(keep interface consistent)
    ...
    # Concurrently generate two images
    html_list = [
        (card_html, 700, 1100),
